        ...
        for i in range(len(self.my_map)):
            for j in range(len(self.my_map[0])):
                if self.my_map[i][j]:
                    ...
                    if (i+1)<len(self.my_map) and (j+1)<len(self.my_map[0]) and (i-1)>0 and (j-1)>0:

                        if ~(self.my_map[i+1][j+1]) or ~(self.my_map[i][j+1]) or ~(self.my_map[i+1][j]) or ~(self.my_map[i-1][j-1]) or ~(self.my_map[i][j-1]) or ~(self.my_map[i-1][j]) or ~(self.my_map[i-1][j+1]) or ~(self.my_map[i+1][j-1]):
                            self.patches.append(Rectangle((i - 0.5, j - 0.5), 1, 1, facecolor='red', edgecolor='gray'))

                    elif (i+1)<len(self.my_map) and (j+1)<len(self.my_map[0]):

                        # print(self.my_map[i+1][j+1],self.my_map[i][j+1],self.my_map[i+1][j])

                        if ~(self.my_map[i+1][j+1]) or ~(self.my_map[i][j+1]) or ~(self.my_map[i+1][j]):
                            # print(i,j)
                            self.patches.append(Rectangle((i - 0.5, j - 0.5), 1, 1, facecolor='red', edgecolor='gray'))
